...
-(void)resetMutableTableViewCellStatus:(UITableView*)tableView rowAtIndexPath:(NSIndexPath*)indexPath{
    ...
    if ([answerMessageDic objectForKey:str] != nil) {
        ...
        if ([mutableArray containsObject:[NSString stringWithFormat:@"%d",row]]) {
...
-(void)resetTableViewCellStatus:(UITableView*)tableView rowAtIndexPath:(NSIndexPath*)indexPath{
    ...
    if ([[dic objectForKey:@"QuestionType"]isEqualToString:@"1"]) {
        ...
        [answerMessageDic setObject:[NSString stringWithFormat:@"%d",row] forKey:str];
...
-(void)willRotateToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration{
    NSLog(@"windows changed %d duration %f",toInterfaceOrientation,duration);
...
//    tableView.contentSize = tableViewSize;
    ...
    if ([[tableViewDic objectForKey:@"QuestionType"] isEqualToString:@"1"]) {
        ...
        if ([[answerMessageDic objectForKey:messageIdStr] isEqualToString:[NSString stringWithFormat:@"%d",row]]) {
    ...
    if ([[tableViewDic objectForKey:@"QuestionType"] isEqualToString:@"2"]){
        ...
        if (([mutableCellArray indexOfObject:[NSString stringWithFormat:@"%d",row]] != NSNotFound)&&(mutableCellArray !=nil)) {
    ...
    if ([[tableViewDic objectForKey:@"QuestionType"] isEqualToString:@"3"]){
        ...
        RatingBar *bar = [[RatingBar alloc] initWithFrame:CGRectMake(90, 0, cell.width/3.0,cell.height)];
...
-(void)coverBtnAction:(id)sender{
    ...
    btn.hidden = YES;
    NSLog(@"%@",suberArray);
    NSLog(@"CoverBtn Action : %d",btn.tag);
...
-(void)setRatingStartStatus:(NSInteger)startNumber atTableViewSection:(NSInteger)currentSection{
    ...
    [answerMessageDic setObject: [NSString stringWithFormat:@"%d",startNumber] forKey:[currentQusetionDic objectForKey:@"Id"]];
...
-(void)textViewBecomeFirstResponder:(UITextView *)textView{
    ...
    NSLog(@"%d",currentSelectedSection);
...
//    textView.tag
    ...
    [self.surveyTableView setContentOffset:CGPointMake(0, rc.y-self.navigationController.navigationBar.height*2) animated:YES];
